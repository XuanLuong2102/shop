...
function ProductItem(props) {
  ...
  const handleAddToCart = (e) => {
    ...
    flying_img.style.setProperty(
      "--topcart",
      `${(cartRect.top - (cartRect.height / 2 + imgRect.height / 2)).toFixed(
        2
      )}px`
    );
    flying_img.style.setProperty(
      "--leftcart",
      `${(cartRect.left + cartRect.width * 2 - imgRect.width / 2).toFixed(2)}px`
    );
      ...
      <div className="product__item">
        <div>
          <div className="product__image">
            ...
            <Link to={`product-detail/${item.id}`} className="product__view">
              View
            </Link>
          </div>
          <div className="content">
            <Link to={`product-detail/${item.id}`} className="product__title">
              {item.title}
            </Link>
